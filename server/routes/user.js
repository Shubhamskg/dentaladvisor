            ...
            if (response?.manual) {
                fs.readFile(`${path.resolve(path.dirname(''))}/mail/template.html`, 'utf8', (err, html) => {
                    if (!err) {
                        ...
                        html = html.replace('[URLS]', `${process.env.SITE_URL}/signup/pending/${response._id}`)
                        html = html.replace('[URLS2]', `${process.env.SITE_URL}/signup/pending/${response._id}`)
                        html = html.replace('[TITLE]', 'Verify your email address')
                        html = html.replace('[CONTENT]', `To continue setting up your ${process.env.SITE_NAME} account, please verify that this is your email address.`)
                        html = html.replace('[BTN_NAME]', 'Verify email address')

                        sendMail({
                            to: req.body.email,
                            subject: `${process.env.SITE_NAME} - Verify your email`,
                            html
                        })
            ...
            if (response) {
                fs.readFile(`${path.resolve(path.dirname(''))}/mail/template.html`, 'utf8', (err, html) => {
                    if (!err) {
                        ...
                        sendMail({
                            to: req.body.email,
                            subject: `Change password for ${process.env.SITE_NAME}`,
                            html
                        })
