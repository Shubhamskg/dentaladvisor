...
const VisionMenu = ({ changeColorMode }) => {
  ...
  useEffect(() => {
    setConfim(false)
    let visionId = path.replace('/vision/', '')
    visionId = visionId.replace('/', '')
    dispatch(activePage(visionId))
  }, [path, history])
  ...
  return (
    <Fragment>
      ...
      <header >
        ...
        <div className='title'>
          {
            path.length > 6 ? history[0]?.prompt : 'New Vision'
          }
        </div>
              ...
              if (obj?.active) {
                return (
                  ...
                  <button 
                    className='active'
                    onClick={() => {
                      navigate(`/vision/${obj?.visionId}`)
                    }}
                  ><Message />
                    {obj?.prompt}
                  </button>
                  <button className="del" onClick={(e)=>{e.preventDefault();deleteChat(obj.visionId);location.reload();}}><Trash className="trash"/></button>
                    </div>
                )
              } else {
                return (
